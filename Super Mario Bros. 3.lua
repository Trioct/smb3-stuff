...
function display_hitboxes()
	...
	for i=1, sprite_slots, 1 do
		...
		sprites[i][3] = memory.readbyte(ram_sprite_state + i) --get sprite state (alive, dead, etc.)
		sprites[i][4] = memory.readbyte(ram_x + i)			  --get x
		sprites[i][5] = memory.readbyte(ram_y + i)			  --get y
		sprites[i][6] = memory.readbyte(ram_low_x + i)		  --get sub x
		sprites[i][7] = memory.readbyte(ram_low_y + i)		  --get sub y
		
		local sprite_x = (memory.readbyte(ram_high_x + i) * 256) + sprites[i][4] --get specific sprite x (without sub pixel)
		local sprite_y = (memory.readbyte(ram_high_y + i) * 256) + sprites[i][5] --get specific sprite y
		if (sprites[i][3] ~= 0) and ((sprite_x - 1 > screen[0]) and (sprite_x + 1 < screen[1]) and (sprite_y - 1 > screen[2]) and (sprite_y + 1 < screen[3])) then --check if within screen, I add or subtract one to be sure
			...
			gui.drawtext(((rect[0] + rect[2]) / 2) - 5, rect[3] - 8, string.format("[" .. i-1 .. "]"), "white", "#00000066") --draw the sprite id above it
		end
	end
end
...
function display_spriteslots()
	...
	for i=1, sprite_slots, 1 do
		if memory.readbyte(ram_sprite_state + i) ~= 0 then --if you need sprite information to display after the sprite's despawn, change the 0 to -1 in this statement
			gui.drawtext(1, y_counter, string.format(i-1), text_color, text_back_color)
			local sprite_x	   = memory.readbyte(ram_x + i)
			local sprite_y	   = memory.readbyte(ram_y + i)
			local sprite_sub_x = memory.readbyte(ram_low_x + i)
			local sprite_sub_y = memory.readbyte(ram_low_y + i)
			gui.drawtext(9, y_counter, string.format("%s%d%s%s%X%s%d%s%s%X%s", "(", sprite_x, ".", (sprite_sub_x == 0 and "0" or ""), sprite_sub_x, ", ", 
																					sprite_y, ".", (sprite_sub_y == 0 and "0" or ""), sprite_sub_y, ")"), text_color, text_back_color) --draw the text
