...
local sprite_slots    = 7 --8 minus mario
...
function display_hitboxes()
    ...
    local screen = {}
    screen[0]    = ((memory.readbyte(ram_high_x) * 256) + memory.readbyte(ram_x)) -  memory.readbyte(ram_relative_x)          --left side
    screen[1]    = ((memory.readbyte(ram_high_x) * 256) + memory.readbyte(ram_x)) - (memory.readbyte(ram_relative_x) - 0xff) --right side
    screen[2]    = ((memory.readbyte(ram_high_y) * 256) + memory.readbyte(ram_y)) -  memory.readbyte(ram_relative_y)          --top
    screen[3]    = ((memory.readbyte(ram_high_y) * 256) + memory.readbyte(ram_y)) - (memory.readbyte(ram_relative_y) - 0xff) --bottom
    
    --find screen if mario is off-screen. Very hacky solution, don't try to analyze this garbage, please.
    for i=0, 3, 1 do
        if screen[i] > 0x7fff then --if high y value has wrapped
            screen[i] = screen[i] - 0xffff --remove high y value, and probably then some
        end
        ...
        while screen[i] < 0 do --loop through the screen sides
            ...
            if pair == 0 then --if zero, it's the first partner (0 or 2)
                pair = 1
            else --else it's the second partner (1, 3)
                pair = -1
            end
            screen[i + pair] = screen[i + pair] + 0xff + (loops > 0 and 1 or 0) --add to the pair as well (horizontal or vertical partner)
            loops = loops + 1
        end
    end
    
    --gui.drawtext(64, 64, string.format(screen[0] .. " " .. screen[1] .. " " .. screen[2] .. " " .. screen[3]))
    
    local y_counter = 24 --for listing sprites and removing blank sprite slot's spaces
    ...
    for i=sprite_slots-1, 0, -1 do
        ...
        sprites[i][4] = memory.readbyte((ram_x + sprite_slots) - i)
        sprites[i][5] = memory.readbyte((ram_y + sprite_slots) - i)
        sprites[i][6] = memory.readbyte((ram_low_x + sprite_slots) - i)
        sprites[i][7] = memory.readbyte((ram_low_y + sprite_slots) - i)
        
        if toggle_display_hitboxes then
            local sprite_x = (memory.readbyte((ram_high_x + sprite_slots) - i) * 256) + sprites[i][4] --get specific sprite x
            ...
            if (sprites[i][3] ~= 0) and ((sprite_x - 1 > screen[0]) and (sprite_x + 1 < screen[1]) and (sprite_y - 1 > screen[2]) and (sprite_y + 1 < screen[3])) then --check if within screen, I add or subtract one to be sure
                local hitbox = AND((rom.readbyte(rom_sprite_attributes + sprites[i][0])), 0x0F) * 4 --check the rom for the hitbox specified by the attributes table + the sprite id. Get the last 4 bits
                                                                                                    --multiply by 4 for searching the table after this
                local rect = {}
        ...
        if toggle_display_sprite_information then
            if (sprites[i][3] ~= 0) then --if you need sprite information to display after the sprite's despawn, change the 0 to -1 in this statement
                ...
                gui.drawtext(9, y_counter, string.format("%s%d%s%s%X%s%d%s%s%X%s", "(", sprites[i][4], ".", (sprites[i][6] == 0 and "0" or ""), sprites[i][6], ", ", 
                                                                                        sprites[i][5], ".", (sprites[i][7] == 0 and "0" or ""), sprites[i][7], ")"), text_color, text_back_color)
...
function post_corner_boost()
    local x_expected = x_total()
    local x_actual = x_prev + (x_speed_prev * 16)
    local x_difference = x_expected - x_actual
    pixel_boost_total = pixel_boost_total + (x_difference == 256 and 1 or 0)
    pixel_boost_negative_total = pixel_boost_negative_total + (x_difference == -256 and 1 or 0)
    gui.drawtext(1, 9, "Pixel Boost Pos: " .. pixel_boost_total .. "; Neg: " .. pixel_boost_negative_total, text_color, text_back_color)
end

--convert frames to hours::minutes::seconds.centiseconds, for example 02:15:34:97 is 2 hours, 15 minutes, and 34.97 seconds
function display_time()
    local frames      = movie.framecount()
    ...
    gui.drawtext(198, 224, string.format((hours < 10 and "0" or "") .. hours .. ":" .. (minutes < 10 and "0" or "") .. minutes .. ":" .. 
                                         (seconds < 10 and "0" or "") .. seconds .. "." .. (centiseconds < 10 and "0" or "") .. centiseconds), text_color, text_back_color)
end
...
function display_information()
    ...
    if toggle_display_mario_velocity then
        gui.drawtext(1, y_counter, string.format("Speed: (" .. memory.readbyte(ram_x_speed) .. ", " .. memory.readbytesigned(ram_y_speed) .. ")"), text_color, text_back_color)
        y_counter = y_counter + 8
    end
