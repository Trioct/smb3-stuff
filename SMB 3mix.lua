...
local ram_is_crouching       = 0x0565 --set if mario is crouching
...
local ram_low_x              = 0x0747 --mario's subpixel x (16 subpixels in a pixel)
...
local ram_low_y              = 0x0759 --mario's subpixel y (16 subpixels in a pixel)
local ram_relative_y         = 0x00B4 --mario's y position on the screen
local ram_y_speed            = 0x00CF --mario's y speed
local ram_sprite_id          = 0x0656 --list of the id's of currently loaded, or once loaded sprites
local ram_sprite_state       = 0x0646 --the states of those sprites (0 = dead)
local ram_rng                = 0x077B --Random Number Generator
local ram_8_frame_timer      = 0x0554 --8 frame timer, 0-7
local ram_p_meter            = 0x03C6 --P meter
local ram_next_p             = 0x0505 --countdown timer until the next p arrow fills up
local ram_p_kill_counter     = 0x0564 --countdown timer until p speed expires
...
local ram_tile_attr_table    = 0x7CE3 --stores 8 bytes which determine whether a tile is solid
...
local ram_spec_obj_low_x     = 0x05B3 --special object low x 
local ram_spec_obj_low_y     = 0x05AB --special object low y
local ram_spec_obj_high_y    = 0x7FD5 --special object high y
local ram_spec_obj_id        = 0x0440 --special object_id

--all of the rom addresses I need
local rom_sprite_attributes = 0x033B --table containing sprite information
local rom_sprite_hitboxes   = 0x02FB --table storing all 16 possible hitboxes
local rom_mario_hitbox      = 0x1A24 --mario's hitbox

local nes_framerate   = 60.0988138974405 --the NES's "exact" frame rate
local sprite_slots    = 7 --8 - mario
local spec_obj_slots  = 8
...
function draw_level()
    ...
    --vertical level, horizontal, or whatever 0xA0 means ;)
    if (level_type == 0x80) or (level_type == 0xC0) or (level_type == 0xA0) then
        ...
        if level_type == 0x80 then --if level is vertical
            ...
            y_offset     = math.floor(((screen_height * 16) - level_height) / 2) --center the level along the y axis
            ...
            x_offset     = math.floor(((screen_width * 16) - level_width) / 2) --center the level along the x axis
        ...
        if toggle_display_mario_in_level then
            ...
            gui.drawpixel(x_offset + (memory.readbyte(ram_high_x) * 16) + math.floor((memory.readbyte(ram_x) + 6) / 16), 
                          y_offset + (memory.readbyte(ram_high_y) * 16) + math.floor((memory.readbyte(ram_y) + 6) / 16) + mario_y_offset, level_mario_color)
            if (memory.readbyte(ram_mario_suit) ~= 0) and (is_crouching == 0) then --if isn't small and not crouching, display extra pixels below mario
                gui.drawpixel(x_offset + (memory.readbyte(ram_high_x) * 16) + math.floor((memory.readbyte(ram_x) + 6) / 16), 
                              y_offset + (memory.readbyte(ram_high_y) * 16) + math.floor((memory.readbyte(ram_y) + 6) / 16) + mario_y_offset + 1, level_mario_color)
            end
        end
